...
export default function AcceptInvitePage() {
  ...
  async function handleAccept(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/invites/accept`, {
        method: "POST",
        headers: {
  ...
  if (valid === null) {
    return <div className="min-h-screen bg-black text-white grid place-items-center">Checking invite...</div>;
  }

  if (!valid) {
    return <div className="min-h-screen bg-black text-red-300 grid place-items-center">{message}</div>;
  }

  return (
    <div className="min-h-screen bg-black text-white grid place-items-center p-6">
      <form onSubmit={handleAccept} className="w-full max-w-md space-y-4 rounded-2xl border border-white/10 bg-white/5 p-8">
        ...
        <input
          ...
          className="w-full rounded-lg bg-black/50 border border-white/10 px-4 py-3"
          required
        />
